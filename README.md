...
弥生取込CSVを選ぶと、CSVステータスは `精算済み` に固定され、画面下部の表示も `弥生取込対象` または `弥生出力済み` へ寄せます。未精算、下書き、取消済みの明細を弥生取込対象に混ぜないための運用ガードです。

Dashboard の `CSV未出力` / `弥生CSV未出力` も、弥生取込と同じく `精算済み` かつ `CSV未出力` の明細を対象にします。確認用CSVの未精算チェックと、弥生取込CSVの会計取込チェックを混同しないためです。
