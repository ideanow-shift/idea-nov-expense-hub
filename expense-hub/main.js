...
async function bootSignedInApp() {
  ...
  if (!currentEmployee) {
    ...
    showAuthMessage(
      "warning",
      "社員情報を確認できません",
      "Supabase Authにはログインできましたが、Expense Hubで使う社員情報に紐づいていません。社員マスタのメール一致、経理ロール、利用対象店舗/部署の設定を管理者に確認してください。"
    );
    return;
  }
...
function showSignedOut() {
  ...
  showAuthMessage(
    "info",
    "経理スタッフのログイン確認",
    "ログインできない場合は、入力メール、Supabase Authユーザー、社員マスタのメール一致、経理ロール付与の4点を確認してください。"
  );
  ...
  try {
    ...
    if (error) {
      showAuthMessage(
        "warning",
        "ログインできませんでした",
        "メールアドレス、パスワード、Supabase Authユーザー発行状況を確認してください。解決しない場合は、入力したメールを管理者へ共有してください。"
      );
      return;
    }
    await bootSignedInApp();
...
function showAuthMessage(type, title, body) {
    ...
    return `
      <article class="monthly-row monthly-review-row" data-monthly-report-id="${escapeHtml(report.id)}">
        ...
        <div class="monthly-review-actions">
          <button type="button" data-monthly-action="approve" data-monthly-report-id="${escapeHtml(report.id)}" ${canApprove ? "" : "disabled"}>内容確認済み</button>
...
function renderReceiptEvidence(row) {
  const receipts = Array.isArray(row.expense_receipts)
    ? row.expense_receipts.filter((receipt) => receipt?.id)
    : [];

  if (!receipts.length) {
    return `<div class="receipt-evidence is-missing">証憑画像: 未添付</div>`;
  }

  return `
    <div class="receipt-evidence">
      <span>証憑画像 ${receipts.length}件</span>
      <div class="receipt-evidence-actions">
        ${receipts.map((receipt, index) => `
          ...
          >${escapeHtml(receiptEvidenceLabel(receipt, index))}</button>
        `).join("")}
      </div>
    </div>
  `;
...
function receiptEvidenceLabel(receipt, index) {
  const fileName = String(receipt?.file_name || "").trim();
  if (fileName) return fileName.length > 24 ? `${fileName.slice(0, 21)}...` : fileName;
  const mimeType = String(receipt?.mime_type || "").toLowerCase();
  if (mimeType.includes("pdf")) return `PDF ${index + 1}`;
  return `レシート ${index + 1}`;
}
...
function bindClaimActions() {
  ...
  claimList.querySelectorAll("button[data-receipt-open]").forEach((button) => {
    button.addEventListener("click", async () => {
      await openReceiptEvidence(button);
    });
  });
}

async function openReceiptEvidence(button) {
  const claim = claimsCache.find((row) => row.id === button.dataset.claimId);
  const receipt = (claim?.expense_receipts || []).find((row) => row.id === button.dataset.receiptId);
  if (!receipt?.storage_path) {
    alert("証憑画像の保存先を確認できませんでした。申請者へ再添付を依頼してください。");
    return;
  }

  const originalLabel = button.textContent;
  button.disabled = true;
  button.textContent = "証憑を読込中...";
  let objectUrl = "";
  ...
  try {
    ...
    if (!opened) {
      ...
      link.target = "_blank";
      link.rel = "noopener";
      link.download = receipt.file_name || "receipt-evidence";
    ...
    if (objectUrl) URL.revokeObjectURL(objectUrl);
    alert(`証憑画像を開けませんでした: ${error.message || String(error)}`);
  } finally {
...
function canCancelClaim(claim) {
  return isCancelClaimEligibleHint({
    claim,
    currentEmployee,
    exported: Boolean(claim?.id && exportedClaimCache.has(claim.id)),
  });
}
...
async function runBulkWorkflowAction(action) {
  if (action === "cancel") {
    alert("一括取消は安全なEdge経路への切替確認中です。個別取消を利用してください。");
    return;
  }
